...
import { getFirestore, doc, onSnapshot, setDoc, enableNetwork, disableNetwork } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
...
// Player colors - High contrast for easy distinction
const PLAYER_COLORS = [
...
async function initializeGame() {
    ...
    try {
        ...
        onSnapshot(gameDocRef, (doc) => {
            if (pauseSync) return; // Skip updates during reset
...
async function syncToFirebase() {
    ...
    try {
        ...
        updateSyncStatus('✅ Synced', 'online');
        saveLocalGameState(); // Also save locally as backup
    } catch (error) {
...
function showView(viewName) {
    ...
    if (viewName === 'map') {
        ...
        document.getElementById('warRoomView').style.display = 'block';
        document.getElementById('warRoomBtn').classList.add('active');
        updateWarRoomView();
...
function showStats(statsType) {
    ...
    document.getElementById(statsType + 'Btn').classList.add('active');
...
function showLogRoundModal() {
    ...
    document.getElementById('logRoundModal').style.display = 'block';
}
...
function logRound() {
    ...
    const birdieHoles = [];
    for (let i = 1; i <= 18; i++) {
        if (document.getElementById(`hole${i}`).checked) {
            birdieHoles.push(i);
        }
    }
    ...
    birdieHoles.forEach(hole => {
        document.getElementById(`hole${hole}`).checked = false;
    });
...
function savePlayerNames() {
    gameState.players.forEach(player => {
        const newName = document.getElementById(`editName${player.id}`).value.trim();
...
// FIXED Admin-protected reset function
async function adminResetGame() {
    ...
    // Pause real-time sync during reset
    pauseSync = true;
    ...
    // Get all player names BEFORE creating players
    const playerNames = [];
    ...
    // Now create players with final names
